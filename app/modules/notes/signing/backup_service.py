...
class ImmutableBackupService:
    ...
    async def generate_institutional_backup(
        self, organization_id: str, executor_id: str, mode: str = "FULL"
    ):
        """
                ...
                "snapshots": [
                    {
                        "id": str(s.id),
                        "hash": s.content_hash,
                        "retention": str(s.retention_until),
                    }
                    for s in data["snapshots"]
                ],
                "seals": [
                    {"id": str(s.id), "hash": s.aggregate_hash} for s in data["seals"]
                ],
                "audit_chain": [
                    {"id": str(a.id), "hash": a.entry_hash} for a in data["audit_log"]
                ],
                "keys": [
                    {"fingerprint": k.public_key_fingerprint, "active": k.is_active}
                    for k in data["keys"]
                ],
            },
        ...
        job.backup_hash = sha256_hex(
            canonical_manifest + (job.previous_backup_hash or "")
        )
        ...
        job.certification_signature = sign_hash(
            load_private_key(priv_pem), job.manifest_hash
        )
        ...
        async with self.db.begin():
            ...
            job.retention_until = datetime.utcnow() + (
