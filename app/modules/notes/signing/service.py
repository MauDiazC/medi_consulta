...
class SigningApplicationService:
    ...
    async def _record_idempotency(self, key: str, payload: dict):
        ...
        ik.retention_until = self.retention_engine.calculate_retention_period(
            1
        )  # short retention
        self.db.add(ik)
    ...
    async def get_snapshot(
        self, note_id: str, requestor_id: str, purpose: str = "review"
    ) -> NoteSnapshot:
    ...
    async def rotate_organization_key(
        ...
        new_key = OrganizationKey(
            ...
            is_active=True,
            # Phase 1 & 3: Retention
            retention_until=self.retention_engine.calculate_retention_period(
                10
            ),  # Long retention for keys
            retention_source="Institutional Key Policy",
            created_at=now,
        )
        await self.signing_repo.save_organization_key(new_key)
        await audit_log(
            self.db, "organization_key", str(new_key.id), "rotated", executor_id
        )
    ...
    async def execute_signing(
        ...
        prof_info = {
            ...
            "professional_license": user.get("professional_license", "PENDING"),
            "role": user["role"],
        }
        ...
        latest = await self.signing_repo.get_latest_snapshot_for_encounter(
            str(note["encounter_id"])
        )
        previous_hash = latest.content_hash if latest else None

        payload, content_hash = build_clinical_snapshot(
            note, version, prof_info, previous_snapshot_hash=previous_hash
        )
        ...
        if idempotency_key:
            await self._record_idempotency(idempotency_key, response)

        await self.db.commit()
        return response

    async def execute_encounter_sealing(
        self,
        encounter_id: str,
        signer_id: str,
        private_key_pem: bytes = None,
        ...
        if await self.signing_repo.get_seal(encounter_id):
        ...
        if private_key_pem:
            ...
            active_key = await self.signing_repo.get_active_organization_key(
                str(org_id)
            )
            if not active_key:
        ...
        snapshots = await self.signing_repo.get_all_snapshots_for_encounter(
            encounter_id
        )
        if not snapshots:
        ...
        seal_payload = {
            ...
            "snapshot_hashes": [
                {"snapshot_id": str(s.id), "content_hash": s.content_hash}
                for s in snapshots
            ],
        ...
        await publish_event_tx(
            self.db,
            "encounter.sealed",
            {"encounter_id": str(encounter_id), "seal_id": str(seal.id)},
        )
