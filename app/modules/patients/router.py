...
async def create_patient(
    payload: PatientCreate, user=Depends(get_current_user), s=Depends(get_service)
):
...
async def activate_patient(
    ...
    await s.repo.db.execute(
        text(
            "UPDATE patients SET is_active=true WHERE id=CAST(:id AS UUID) AND organization_id=CAST(:org AS UUID)"
        ),
