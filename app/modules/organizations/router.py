...
def get_service(db=Depends(get_db)):
    return OrganizationService(OrganizationRepository(db), UserRepository(db))
...
async def debug_context(
    ...
    r = await db.execute(
        text(
            "SELECT id, email, role, organization_id, active FROM users WHERE id = CAST(:id AS UUID)"
        ),
        {"id": user["sub"]},
    )
    ...
    audit_r = await db.execute(
        text(
            "SELECT id, email, role, organization_id FROM users WHERE id IN (SELECT CAST(id AS UUID) FROM (VALUES (:id1), (:id2), (:id3)) as t(id))"
        ),
    ...
    e_user = await db.execute(
        text(
            "SELECT id, organization_id, doctor_id, status, created_at FROM encounters WHERE doctor_id = CAST(:id AS UUID)"
        ),
        {"id": user["sub"]},
    )
    ...
    e_global = await db.execute(
        text("SELECT id, doctor_id, organization_id, status FROM encounters LIMIT 5")
    )
    ...
    p_check = await db.execute(
        text(
            "SELECT COUNT(*) as count FROM patients WHERE organization_id = CAST(:oid AS UUID)"
        ),
        {"oid": user["org"]},
    )
...
async def create_org(
    ...
    if user.get("email") != "mdiazcabr@gmail.com":
        raise HTTPException(
            status_code=403,
            detail="Only global administrators can create organizations manually.",
        )
...
@router.get("/{org_id}")
async def get_org(org_id: str, user=Depends(get_current_user), s=Depends(get_service)):
...
async def deactivate_org(
    org_id: str, user=Depends(require_role("admin")), s=Depends(get_service)
):
    """SaaS Guard: Only global admin can deactivate entire organizations."""
    if user.get("email") != "mdiazcabr@gmail.com":
        raise HTTPException(
            status_code=403, detail="Only global admins can deactivate organizations."
        )
...
async def activate_org(
    ...
    if user.get("email") != "mdiazcabr@gmail.com":
        raise HTTPException(
            status_code=403, detail="Only global admins can activate organizations."
        )
