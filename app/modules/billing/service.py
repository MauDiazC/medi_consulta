...
class BillingService:
    ...
    async def create_portal_session(self, org_id: str, return_url: str):
        ...
        if not customer_id:
            raise HTTPException(
                400, "No active subscription found for this organization"
            )
    ...
    async def _process_invoice_paid(self, invoice):
        ...
        if org:
            ...
            await self.org_repo.sync_subscription_status(
                org_id=org_id, status="active", period_end=period_end
            )
    ...
    async def _process_subscription_sync(self, subscription):
        ...
        if org:
            org_id = str(org["id"])
            await self.org_repo.sync_subscription_status(org_id=org_id, status=status)

            # Create Outbox Event for sync
            outbox_item = OutboxEvent(
                event_type="billing.subscription_updated",
                payload={"organization_id": org_id, "status": status},
            )
            self.org_repo.db.add(outbox_item)
                    ...
                    "items": [
                        {
                            ...
                            "product": {
                                ...
                                / 100.0,  # Stripe cents to decimals
