...
from sqlalchemy import JSON, Boolean, Column, DateTime, String, event
    ...
    def validate_modification_safety(self):
        if self.is_immutable:
            raise StructuralIntegrityError(
                f"Structural Violation: Entity {self.__class__.__name__} is immutable."
            )
        if self.legal_hold:
            raise StructuralIntegrityError(
                f"Structural Violation: Entity {self.__class__.__name__} is under Legal Hold."
            )
        if self.retention_until and datetime.now(UTC) < self.retention_until:
            raise StructuralIntegrityError(
                f"Structural Violation: Retention has not expired for {self.__class__.__name__}."
            )
...
def block_all_physical_deletes(mapper, connection, target):
    raise StructuralIntegrityError(
        f"Structural Violation: Physical deletion prohibited for {target.__class__.__name__}."
    )
...
def session_integrity_guard(session, flush_context, instances):
    ...
    for obj in session.deleted:
        if isinstance(obj, ImmutableLegalArtifact):
            raise StructuralIntegrityError(
                f"Structural Violation: Deletion intercepted at session level for {obj.__class__.__name__}."
            )
